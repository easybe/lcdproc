...
void spi_sevenseg_senddata(PrivateData *p, unsigned char display_id, unsigned char flags, unsigned char ch);
void spi_sevenseg_backlight(PrivateData *p, unsigned char state);
...
spi_transfer(PrivateData *p, const unsigned char *outbuf, unsigned char *inbuf, unsigned length)
{
	...
	p->sevenseg_functions->drv_debug(RPT_DEBUG, "SPI sending %02x %02x %02x",
					outbuf[0], outbuf[1], length > 2 ? outbuf[2] : 0xFFu);
	status = ioctl(p->fd, SPI_IOC_MESSAGE(1), &xfer);
	if (status < 0) {
		p->sevenseg_functions->drv_report(no_more_errormsgs ? RPT_DEBUG : RPT_ERR,
						 "SEVENSEG: SPI: spidev write data %u failed: %s",
						 status, strerror(errno));
		no_more_errormsgs = 1;
	}
#ifdef DEBUG
	if (inbuf)
		p->sevenseg_functions->drv_debug(RPT_DEBUG, "SPI returned data %02x %02x %02x",
 ...
 */
int
sevenseg_init_spi(Driver *drvthis)
{
	...
	report(RPT_INFO, "SEVENSEG: SPI: Using device '%s'", device);

	p->fd = open(device, O_RDWR);
	if (p->fd < 0) {
		report(RPT_ERR, "SEVENSEG: SPI: open spidev device '%s' failed: %s", device,
		       strerror(errno));
		return -1;
	}
	...
	if (strlen(backlight_device) > 0) {
		report(RPT_INFO, "SEVENSEG: SPI: Using backlight_device '%s'", backlight_device);

		p->backlight_bit = open(backlight_device, O_RDWR);
		if (p->backlight_bit < 0) {
			report(RPT_ERR, "SEVENSEG: SPI: open backlight_device '%s' failed: %s",
			       backlight_device, strerror(errno));
		}
		else {
			sevenseg_functions->backlight = spi_sevenseg_backlight;
		}
	}

	sevenseg_functions->senddata = spi_sevenseg_senddata;
 ...
 * \param p          Pointer to driver's private data structure.
 * \param display_id ID of the display (or 0 for all) to send data to.
 * \param flags      Defines whether to end a command or data.
 * \param ch         The value to send.
 */
void
spi_sevenseg_senddata(PrivateData *p, unsigned char display_id, unsigned char flags, unsigned char ch)
{
	unsigned char buf[3];
	unsigned char reverse;

	p->sevenseg_functions->drv_report(RPT_DEBUG, "SEVENSEG: SPI: sending %s %02x",
 ...
 */
void
spi_sevenseg_backlight(privatedata *p, unsigned char state)
{
	if (p->backlight_bit != -1) {
		unsigned char byte = (state == BACKLIGHT_ON) ? '1' : '0';
		if (write(p->backlight_bit, &byte, sizeof(byte)) < 0)
			p->sevenseg_functions->drv_report(RPT_ERR,
							 "SEVENSEG: SPI: Cannot write to backlight device: %d (%s)",
